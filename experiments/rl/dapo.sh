...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-simpleqa_dapo}
...
# DAPO uses a dedicated generation batch (oversample), then filters groups down
# to the train batch. GEN_PROMPT_BSZ defaults to 3x TRAIN_BATCH_SIZE to give the
# group filter headroom to drop all-correct / all-wrong groups.
TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE:-64}
GEN_PROMPT_BSZ=${GEN_PROMPT_BSZ:-$((TRAIN_BATCH_SIZE * 3))}
...
# DAPO-specific knobs (paper defaults).
...
# overlong_buffer.len is a soft-penalty window carved off the end of RESP_LEN.
# Default to min(512, RESP_LEN/4) so it works for both short/long response budgets.
OVERLONG_BUFFER_LEN=${OVERLONG_BUFFER_LEN:-$(( RESP_LEN / 4 ))}
...
")
	...
	CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" python3 -m recipe.dapo.main_dapo \
	...
	# (time budget / ESI expiration). Bail out with nonzero rc so
	# chain_job.sh's --complete-on-success does NOT mark the chain done,
	# and the next chain iteration resumes from the saved checkpoint.
