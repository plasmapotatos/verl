#!/usr/bin/env bash
# Chain the unbracketed_question/original SFT run, and on completion chain the
# matching binary RL run that trains on top of the last SFT checkpoint.
set -euo pipefail

WORKDIR="/work/hdd/bbsg/twei2/rl/verl"
SFT_SCRIPT="$WORKDIR/experiments/factual_anchor/unbracketed_question/original/sft/train_sft.sh"
RL_SCRIPT="$WORKDIR/experiments/factual_anchor/unbracketed_question/original/rl/binary.sh"
